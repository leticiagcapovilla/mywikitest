...
#### C/C++
Makefiles must be provided, with targets *all* and *install* for compiling and installing C/C++ applications, respectively. On Linux, the *install* target must install files on <code>/usr/local/<bin|include|lib></code>. Additionally, a *develop* target may be provided; this should install symbolic links to the files in the development directory, so that any changes made are immediately available for development and testing purposes.

#### Python
Packages must include a <code>setup.py</code> file, using [setuptools](https://bitbucket.org/pypa/setuptools). Dependencies must not be defined in this file for automatic download and installation, but be listed in the INSTALL file instead.
